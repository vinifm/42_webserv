	...
	std::vector<std::string>::iterator& line)
{
	...
	if (prefix[0] != '/')
		throw std::runtime_error("location: invalid prefix");
	_prefix = prefix;
...
void	Location::_setRoot(std::stringstream& ss)
{
	std::string	path;
	if (!(ss >> path))
		throw std::runtime_error("locatin: missing root path");
	char lastChar = path[path.length() - 1];
	if (lastChar != '/')
		throw std::runtime_error("location: root: path is not a directory");
	_root = path;
}
