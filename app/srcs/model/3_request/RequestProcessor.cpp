...
std::string	 Request::execute_get(Parser& parser, Response& response)
{
	...
	location = set_last_root(parser, this->_last_root, route);

	//3)APPEND ROOT + ROUTE:
	last_root_and_route.str("");
	this->_last_root = add_final_bar(this->_last_root);
	last_root_and_route << this->_last_root << route;
	ss.str(""); ss << "("<< this->_last_root << "|"<< route <<") full request route ("<< last_root_and_route <<")"; print_log("RequestProcessor.cpp", ss.str(), 1);
